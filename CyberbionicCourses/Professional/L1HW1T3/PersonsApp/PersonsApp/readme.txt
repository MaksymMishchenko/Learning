...
+6. Создать метод RemoveFirst() который удаляет гражданина с начала коллекциию.
+7. Создать метод Remove(Citizen citizen) который удаляет гражданина любого типа гражданина.
+8. Создать метод Contains, в конструкции for пройтись по коллекции. Если элемент есть в наличии, возвращаем true и индекс - порядковый номер элемента;
+9. Создать метод ReturnLast, который обращается к последнему элементу коллекции, возвращает последний элемент и индекс;
+10. Создать метод Clear, который присваивает массиву значение пустого;
+11. Реализовываем члены Current, MoveNext, Reset, GetEnumerator;
+12. В методе Main() создаем экземпляры классов Collection, Student, Retired, Worker. В блоке инициализатора добавляем данные граждан.
