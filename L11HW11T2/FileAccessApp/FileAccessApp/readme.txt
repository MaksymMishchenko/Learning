...
-1. Создать класс-обьект блокировки BlockObject в котором симитировать работу методов Monitor.Enter() и Monitor.Exit:
 ...
 - проинициализировать переменную wait в конструкторе значением аргумента;
  - метод Enter():
   ...
   - если переменная result равна 1, тогда заходим в цикл while, вызываем метод Sleep на классе-обьекте Thread;
   - переменной result присваиваем возвращаемое значение метода CompareExchange(); 
  - метод Exit():
   - присваиваем значение переменной result = 0 вызвав метод на классе-объекте Interlocked.CompareExchange() - высвобождая поток;
